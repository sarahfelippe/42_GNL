...
/*   Updated: 2022/10/11 04:38:33 by sfelippe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*line_before_break;
	char		*return_line;

	if (fd == -1 || BUFFER_SIZE <= 0)
		return (NULL);
	line_before_break = ft_read_join_line_until_break(fd, line_before_break);
	if (!line_before_break)
		return (NULL);
	return_line = ft_get_line_until_break(line_before_break);
	line_before_break = ft_get_line_after_break(line_before_break);
	return (return_line);
}
